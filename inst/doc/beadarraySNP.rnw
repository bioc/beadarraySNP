...
pd<-coerce(read.phenoData(paste(datadir,"targets.txt",sep="/"),sep="\t"),new("AnnotatedDataFrame"))
pData(SNPdata)<-cbind(pData(SNPdata),pData(pd))
ld<-c(as.character(varMetadata(SNPdata)$labelDescription),as.character(varMetadata(pd)$labelDescription))
rn<-c(rownames(varMetadata(SNPdata)),rownames(varMetadata(pd)))
varMetadata(SNPdata) <- data.frame(labelDescription=ld,row.names=rn)
@
...
Normalization to calculate copy number is a multi-step process. In Ref. we have 
